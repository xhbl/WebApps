...
let timerStartTime = 0; // 计时器开始时间（用于精确计时）
...
function loadGame() {
    ...
    try {
        ...
        // 恢复计时器（设置 timerStartTime 以便恢复后继续准确计时）
...
function placeMines(firstRow, firstCol) {
    ...
    // 放置雷（确保不超过可用格子数）
    const minesToPlace = Math.min(gameState.config.mines, availableCells.length);
...
function startTimer() {
    ...
    gameState.timerInterval = setInterval(() => {
        ...
        const newTimer = Math.min(elapsed, 999);
        
        // 只在值变化时更新 DOM
        if (newTimer !== gameState.timer) {
...
// 确保在 DOM 完全加载且布局计算完成后再初始化游戏
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        // 使用 requestAnimationFrame 确保布局已计算
...
function initializeGame() {
    ...
    // 使用事件委托处理最佳时间链接点击（避免内存泄漏）
    DOM.bestTimeDisplay.addEventListener('click', e => {
        const link = e.target.closest('#bestTimeLink');
        if (link) {
            e.preventDefault();
            showAllBestTimes();
        }
