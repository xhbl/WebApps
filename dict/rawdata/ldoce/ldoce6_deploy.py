...
header('X-LDOCE6-Search-Version: 2026-02-19');
...
""" + common_lookup + """
    ...
    if ($result) {
        ...
        $isAudioHref = function($href) {
            ...
            return preg_match('/\.(mp3|wav|spx)(?:$|[?#])/i', $href) === 1;
        };
    ...
    if ($sound_file) {
        ...
        if (!file_exists($absolute_audio_path)) {
            // 尝试不同的路径组合
            $possible_paths = [
                __DIR__ . '/audio/' . basename($decoded_file),
                __DIR__ . '/sounds/' . basename($decoded_file),
                __DIR__ . '/' . basename($decoded_file)
            ];
            
            foreach ($possible_paths as $path) {
                if (file_exists($path)) {
                    $absolute_audio_path = $path;
                    $relative_audio_path = str_replace(__DIR__ . '/', '', $path);
                    break;
                }
            }
        }
...
def create_index_html():
    ...
    builder = HtmlBuilder()
    builder.set_title("朗文当代高级英语词典")
    
    # 添加 Favicon
    builder.head_content.append('<link rel="icon" href="ldoce6.png" type="image/png">')
        ...
        async function search(word, updateHistory = true) {
            ...
            try {
                ...
                if (data.found) {
                    ...
                    // 重新初始化词典脚本
                    if (typeof d8018d6852bc49e3b3e655364cf1439c !== 'undefined') {
...
def create_browser_html():
    ...
    builder = HtmlBuilder()
    builder.set_title("朗文当代高级英语词典 - 浏览器")
...
def copy_favicon():
    ...
    if os.path.exists('ldoce6.png'):
        print("Copying ldoce6.png to root directory (as favicon)...")
        shutil.copy('ldoce6.png', os.path.join(WWW_DIR, 'ldoce6.png'))
    else:
        print("Warning: ldoce6.png not found in current directory.")

def copy_and_patch_assets():
    """复制静态资源 (CSS/JS) 并应用补丁"""
    print("Copying static assets...")
    for asset in ['lm6.css', 'lm6.js']:
...
def main():
    parser = argparse.ArgumentParser(description='Prepare LDOCE6 Dictionary for Apache Deployment')
